...
use super::*;
pub async fn accept_en_tcp(worker_queue: UnboundedSender<Worker>, config: Settings) -> Result<()> {
...
async fn transfer(
    ...
    if stream_type == crate::client::TCP {
        handle_tcp_pool(worker_queue, worker_r, worker_w, &pools, &config, true).await
    } else if stream_type == crate::client::SSL {
        handle_tls_pool(worker_queue, worker_r, worker_w, &pools, &config, true).await
    } else {
